 ...
 *   POST   /agent/drafts/{slug}/install                           → install draft to plugins dir
 */
final class AgentController {
	public static function register(): void {
		...
		register_rest_route( 'examplepress-mu/v1', '/agent/drafts/(?P<slug>[a-z0-9-]+)/install', [
			'methods'             => 'POST',
			'callback'            => [ self::class, 'installDraft' ],
			'permission_callback' => [ self::class, 'permissionCheck' ],
			'args'                => [
				'slug' => [ 'required' => true, 'type' => 'string', 'sanitize_callback' => 'sanitize_title' ],
			],
	...
	public static function retryJob( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		// If the draft was already generated (payload exists), skip
		// Phase 1 entirely and resume at the commit/push step.
		if ( $slug !== '' && AppRegistry::hasDraftPayload( $slug ) ) {
		...
		// No draft yet — re-run the full generation. Preserve every
		// field from the original attempt so the user doesn't have to
		// re-type the app name or error context.
		$result = GenerationJob::enqueue( [
	 ...
	 */
	public static function installDraft( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		if ( $err = self::ensureFeature() ) {
			return $err;
		}
		$slug   = (string) $request->get_param( 'slug' );
		$result = GenerationJob::installLocal( $slug );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
		return rest_ensure_response( [ 'success' => true, 'slug' => $slug ] );
