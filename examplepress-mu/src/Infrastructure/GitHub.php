...
final class GitHub {
	...
	public static function createRepo( string $slug, string $description ): array|\WP_Error {
		...
		if ( $code === 422 && is_array( $body ) && ! empty( $body['errors'] ) && is_array( $body['errors'] ) ) {
			foreach ( $body['errors'] as $err ) {
				if ( is_array( $err ) && ( $err['message'] ?? '' ) === 'name already exists on this account' ) {
					// Repo already exists (e.g. prior attempt created it but
					// the push failed). Fetch its details so the caller can
					// proceed instead of giving up.
					return self::getRepo( $org . '/' . $slug );
				}
			}
		}
		...
		// A GitHub App installation token is scoped to the repos that
		// existed when the token was minted. The repo we just created
		// isn't in that set, so any subsequent API call (pushFiles,
		// createRelease) would 404. Flush the cached token so the next
		// writeToken() call gets a fresh one that covers the new repo.
	...
	/**
	 * Fetch an existing repo's details by owner/repo.
	 *
	 * @return array{owner_repo:string,repo_id:int,html_url:string}|\WP_Error
	 */
	public static function getRepo( string $ownerRepo ): array|\WP_Error {
		$pat = self::writeToken();
		if ( ! $pat ) {
			return new \WP_Error( 'no_github_token', 'No GitHub write token available.' );
		}

		$response = wp_remote_get( "https://api.github.com/repos/{$ownerRepo}", [
			'headers' => [
				'Authorization' => "Bearer {$pat}",
				'Accept'        => 'application/vnd.github.v3+json',
				'User-Agent'    => 'ExamplePress/' . EXAMPLEPRESS_MU_VERSION,
		...
		if ( $code !== 200 || ! is_array( $body ) || empty( $body['full_name'] ) ) {
			$msg = is_array( $body ) && isset( $body['message'] ) ? (string) $body['message'] : "HTTP {$code}";
			return new \WP_Error( 'github_api_error', "Could not fetch repo {$ownerRepo}: {$msg}" );
		}
	...
	public static function pushFiles(
		...
		$pat = self::writeToken();
		if ( ! $pat ) {
			error_log( '[GitHub::pushFiles] No GitHub write token available.' );
			return new \WP_Error( 'no_github_token', 'No GitHub write token available.' );
		}
		...
		// When no parentSha is provided, check if the default branch
		// already has commits (e.g. auto_init README or a prior push).
		// Use its HEAD as the parent so we don't create orphan commits
		// or fail on empty-repo edge cases.
		if ( ! $parentSha ) {
			$refResp = wp_remote_get( "{$base}/git/ref/heads/{$branch}", [
				'headers' => $headers,
				'timeout' => 10,
			] );
			if ( ! is_wp_error( $refResp ) ) {
				...
				if ( ! empty( $refBody['object']['sha'] ) ) {
					$parentSha = (string) $refBody['object']['sha'];
				}
			}
		}
		...
		// Empty repo (no commits at all) — the Git Database API returns
		// 409 "Git Repository is empty." and refuses to create trees.
		// Bootstrap with a single-file commit via the Contents API,
		// then retry with the new commit as parent.
		$treeCode = (int) wp_remote_retrieve_response_code( $treeResp );
		if ( empty( $treeBody['sha'] ) && $treeCode === 409 && ! $parentSha ) {
			$initResp = wp_remote_request( "{$base}/contents/.gitkeep", [
				'method'  => 'PUT',
				'headers' => $headers,
				'body'    => wp_json_encode( [
			...
			if ( ! is_wp_error( $initResp ) ) {
				$initBody  = json_decode( wp_remote_retrieve_body( $initResp ), true );
				$parentSha = (string) ( $initBody['commit']['sha'] ?? '' );
			}
			if ( $parentSha ) {
				$treePayload['base_tree'] = $parentSha;
				$treeResp                 = wp_remote_post( "{$base}/git/trees", [
		...
		error_log( '[GitHub::pushFiles] Creating commit: ' . json_encode( $commitPayload ) );
		$commitResp = wp_remote_post( "{$base}/git/commits", [
			'headers' => $headers,
			'body'    => wp_json_encode( $commitPayload ),
			'timeout' => 30,
		] );
		error_log( '[GitHub::pushFiles] Commit response: ' . print_r( $commitResp, true ) );
		if ( is_wp_error( $commitResp ) ) {
			error_log( '[GitHub::pushFiles] Commit error: ' . $commitResp->get_error_message() );
			return $commitResp;
		}
		$commitBody = json_decode( wp_remote_retrieve_body( $commitResp ), true );
		if ( empty( $commitBody['sha'] ) ) {
			error_log( '[GitHub::pushFiles] Commit failed: ' . print_r( $commitBody, true ) );
			return new \WP_Error( 'commit_failed', $commitBody['message'] ?? 'Failed to create commit.' );
		}

		// Create or update the branch ref.
		if ( ! $parentSha ) {
			error_log( '[GitHub::pushFiles] Creating branch ref for ' . $branch . ' with sha ' . $commitBody['sha'] );
			...
			error_log( '[GitHub::pushFiles] Updating branch ref for ' . $branch . ' with sha ' . $commitBody['sha'] );
		...
		error_log( '[GitHub::pushFiles] Ref response: ' . print_r( $refResp, true ) );
		if ( is_wp_error( $refResp ) ) {
			error_log( '[GitHub::pushFiles] Ref error: ' . $refResp->get_error_message() );
			return $refResp;
		}
		$refCode = wp_remote_retrieve_response_code( $refResp );
		if ( $refCode < 200 || $refCode >= 300 ) {
			$refBody = json_decode( wp_remote_retrieve_body( $refResp ), true );
			error_log( '[GitHub::pushFiles] Ref failed: ' . print_r( $refBody, true ) );
			return new \WP_Error( 'ref_failed', $refBody['message'] ?? "Failed to update branch ref (HTTP {$refCode})." );
		}

		error_log( '[GitHub::pushFiles] Success! Commit SHA: ' . $commitBody['sha'] );
